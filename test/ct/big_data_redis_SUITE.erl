...
not_found(_) ->
    ...
    BigKey = <<"a">>,
    RowID = <<"1">>,
    ?assertEqual(?BD_NOTFOUND, big_data_redis:get(Ref, BigKey)),
    ?assertEqual(?BD_NOTFOUND, big_data_redis:get_row(Ref, BigKey, RowID)),
    ok =
        big_data_redis:insert(Ref,
                            BigKey,
                            #row_data{row_id = <<"2">>,
                                      time = 1,
                                      term = 1}),
    ?assertEqual(nil, big_data_redis:get_row(Ref, BigKey, RowID)),
    ok.
...
remove(_) ->
    ...
    ?assertEqual(?BD_NOTFOUND, big_data_redis:get(Ref, BigKey)),
    ?assertEqual(notfound, big_data_redis:get_row(Ref, BigKey, RowID)),
    ok.
...
clear(_) ->
    ...
    big_data_redis:clear(Ref),
    ?assertEqual(?BD_NOTFOUND, big_data_redis:get_row(Ref, BigKey, RowID)),
    ?assertEqual(?BD_NOTFOUND, big_data_redis:get(Ref, <<"b">>)),
    ok.
    ...
    ok =
        big_data_redis:insert(Ref,
                            ...
                            #row_data{row_id = <<"1">>,
                                      ...
                                      time = 10}),
    [true] =
        big_data_redis:update_elem(Ref, BigKey, <<"1">>, {0, {{a, b}, [1, 2, 3], <<"hello">>, 8}}),
